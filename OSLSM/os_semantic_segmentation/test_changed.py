...
def test_net(loader, model_path, weights_path, output_dir, test_iters, gpu, test_cats_indicator=range(20), save_images=True):
	...
	print(np.min(np.array(a[0])))
	print(np.max(np.array(a[0])))
	print(np.unique(np.array(a[3])))
	print(np.max(np.array(a[3])))
	print(a[-1]['first_index'])
	print(a[-1]['second_index'])
	print(a[-1]['seq_name'])
	
	original_image = Image.open('/root/code/pcbs/source_files/fovs_001.png')
	original_image = np.array(original_image)[:900,:1000,:]
	print("-------------------Size of original image------------------")
	print(np.max(original_image))
	original_image = Image.fromarray((original_image).astype(np.uint8))
	original_image.save('/root/code/pcbs/source_files/fovs_001_scale.png')
	
    	width, height = original_image.size
	first_img = original_image.resize((224,224))
    	width, height = first_img.size
	first_img.save('/root/code/pcbs/source_files/fovs_001_scale.png')
	print(type(first_img))
	print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
	original_image = Image.open('/root/code/pcbs/source_files/fovs_002.png')
	original_image = np.array(original_image)#[800:1700,:1000,:]
	original_image = Image.fromarray((original_image).astype(np.uint8))
    	width, height = original_image.size
	second_img = original_image.resize((500,500))
    	width, height = second_img.size
	second_img.save('/root/code/pcbs/source_files/fovs_002_scale.png')



	first_img = cv2.imread('/root/code/pcbs/source_files/fovs_001_scale.png')-139
	first_img = np.reshape(first_img , (1,3,first_img.shape[0],first_img.shape[1]))

	first_label = np.pad(np.load('/root/code/pcbs/bootstrapped_ground_truth/fovs_001_10.npy'), ((8,8),(8,8)), 'constant')[:900,:1000]
	first_label = np.array(first_label==2,dtype=int)
	print('HMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM')
	print(np.max(first_label))
	#scipy.misc.imsave('/root/code/pcbs/source_files/fovs_001_10.png', first_label)
	first_label = Image.fromarray((first_label*255).astype(np.uint8),'L')
	
	first_label = first_label.resize((224,224))
	first_label.save('/root/code/pcbs/source_files/fovs_001_10_scale.png')
	first_label = cv2.imread('/root/code/pcbs/source_files/fovs_001_10_scale.png',0)/255	#np.array(first_label)/255
	print(np.sum(first_label))
	print(first_label.shape)
	first_label = np.reshape(first_label, (1,1,first_label.shape[0],first_label.shape[1]))

	second_img = cv2.imread('/root/code/pcbs/source_files/fovs_002_scale.png')-139
	second_img = np.reshape(second_img , (1,3,second_img.shape[0],second_img.shape[1]))
	
	print(second_img)
	second_label = np.pad(np.load('/root/code/pcbs/bootstrapped_ground_truth/fovs_002_10.npy'), ((8,8),(8,8)), 'constant')#[800:1700,:1000]
	second_label = np.array(second_label==2,dtype=int)
	print(np.sum(second_label)/25.0)
	#scipy.misc.imsave('/root/code/pcbs/source_files/fovs_002_10.png', second_label)
	second_label = Image.fromarray((second_label*255).astype(np.uint8),'L')
	second_label = second_label.resize((500,500))
	second_label.save('/root/code/pcbs/source_files/fovs_002_10_scale.png')
	second_label = cv2.imread('/root/code/pcbs/source_files/fovs_002_10_scale.png',0) 	
	second_label = np.asarray(second_label)/255
	print("Uniqueeeeeeeeeeee")
	print(np.unique(second_label))
	second_label = np.reshape(second_label, (1,1,second_label.shape[0], second_label.shape[1]))


	print(second_label)
	print(np.sum(second_label))
	...
	#print first_label.shape
	#print np.sum(first_label)
	#print np.unique(first_label)
	#print first_img.shape
	##print first_label[0,0,0].shape
	...
	#print np.sum(second_label)
	print np.unique(second_label) 	
	#print(loader.out['second_label'][0])
	#print(np.unique(loader.out['second_label']))
	#print 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
	#print(second_label[0,0])
	#print(np.unique(second_label[0,0]))
	#print '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&'
	#print loader.out.keys()
	#print loader.out['original_first_label']
	#print np.array(loader.out['original_first_label']).shape
	#print np.array(loader.out['first_label']).shape
	#print deploy_info['first_semantic_labels'][0][0]
	
	loader.out['first_img'] = first_img
	loader.out['first_label'] = first_label
	loader.out['second_img'] = second_img
	loader.out['second_label'] = second_label
	loader.out['deploy_info']['first_img_orig'] = first_img
	loader.out['deploy_info']['second_img_orig'] = second_img
	loader.out['deploy_info']['first_index'] = -1
	loader.out['deploy_info']['second_index'] = -1
	loader.out['deploy_info']['second_semantic_labels'] = 12
	#loader.out['deploy_info']['seq_name'] = 
	loader.out['deploy_info']['second_mask_orig'] = second_label
	loader.out['deploy_info']['first_semantic_labels'] = 12
	loader.out['deploy_info']['first_mask_orig'] = first_label
	
	print(net.inputs)
	print("---------------------Changes---------------------------------")
        inputs = compute_net_inputs(loader.out, net.inputs) # change this totally thats enough ---> what are
	...
	print(np.sum(pred>0.65))
        tp, tn, fp, fn = measure(second_label[0,0], net.blobs['score'].data[0,0])
        iou_img = tp/float(max(tp+fp+fn,1))
        class_ind = 11#int(deploy_info['first_semantic_labels'][0][0])-1 # because class indices from 1 in data layer
